...
@RestController  // @RestController = @Contoller + @ResoponseBody  기능(데이터로만 클라이언트한테 내려감) 
					// @ResoponseBody : View 없이 직접 데이터를 클라이언트에 전송
@RequestMapping("ajax")
public class AjaxController {
	...
	@RequestMapping(value="graph2" , produces="text/plain; charset=UTF8")  //내가 지금보내는데이터는 텍스트형태이고 한글이있다
public String graph2(HttpSession session) {
	//map: {"홍길동",10},{"김삿갓",8} ...
		
		User loginuser =  (User) session.getAttribute("loginUser");

	Map<String,Object> map = service.graph2(loginuser.getName());
	...
	public String exchange1() {
		...
		html.append("<tr><th>기준율</th><th>받을떄</th><th>파실때</th></tr>");
	...
	public String exchange2() {
		...
		try {
	         ...
	         html.append("<th rowsapn='2' style='vertical-align:middle;'>"+"기준율</th>");
